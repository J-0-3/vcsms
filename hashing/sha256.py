# https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.180-4.pdf
# https://sha256algorithm.com/

def circular_right_shift(n: int, b):
    # right shift 32-bit integer n by b and wrap bits around
    bits_to_wrap = (n << (32 - b)) % 2 ** 32
    shifted_bits = n >> b
    return bits_to_wrap | shifted_bits


def sha256(message: bytes, salt: bytes):
    message_block = b''
    message_block += message
    message_block += (0b10000000).to_bytes(1)  # append 1000000 so that it will always pad even if already 464 bits
    while (len(message_block) * 8 + 64) % 512 != 0:  # a 64-bit integer will be added to end so pad to 512*n - 64
        message_block += (0).to_bytes(1)

    message_block += (len(message) * 8).to_bytes(8,
                                                 byteorder='big')  # 64-bit integer containing the length of the message
    print("Message block:")
    for b in message_block:
        print(bin(b)[2:], end='')
    print('\n')

    chunks = []
    for i in range(0, len(message_block), 64):  # split into 512-bit (64 byte) chunks
        chunks.append(message_block[i:i + 64])

    print(chunks)

    # government document says use these wacky magic numbers

    # first 32 bits of the fractional parts of the square roots of the first 8 primes
    h = [0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19]
    # first 32 bits of the fractional parts of the cube roots of the first 64 primes
    k = [0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
         0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
         0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
         0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
         0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
         0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
         0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
         0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2]

    def sigma_zero(x: int) -> int: return circular_right_shift(x, 7) ^ circular_right_shift(x, 18) ^ (x >> 3)
    def sigma_one(x:int) -> int: return circular_right_shift(x, 17) ^ circular_right_shift(x, 19) ^ (x >> 10)
    def add32bits(x:int, y:int) -> int: return (x + y) % 2**32

    for c in chunks:
        words = []
        for i in range(0, len(c), 4):
            words.append(int.from_bytes(c[i:i + 4]))
        for i in range(16, 64):
            words.append((sigma_zero(words[i-2]) + words[i-7] + sigma_zero(words[i-15]) + words[i-16]) % 2 ** 32)

        [print(bin(w).zfill(32)) for w in words]


if __name__ == "__main__":
    sha256(b"hello", b'')
