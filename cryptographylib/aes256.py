import math
from cryptographylib.utils import get_msb, i_to_b
...
if __name__ == "__main__":
    ...
    group.add_argument('-e', '--encrypt', help="encrypt stdin", action="store_true")
    group.add_argument('-d', '--decrypt', help="decrypt stdin", action="store_true")
    parser.add_argument('-x', '--hex', help='format ciphertext input/output as hex', action="store_true")
    group = parser.add_mutually_exclusive_group()
    group.add_argument('--use-ecb', help='use insecure electronic codebook encryption mode')
    group.add_argument('-iv', '--initialisation-vector', help='the cipher block chaining initialisation vector (128 bit hex)')
    parser.add_argument('password', help='the encryption password')
    parser.add_argument('-i', '--infile', help="read input from file")
    parser.add_argument('-o', '--outfile', help="write output to file")
    
    args = parser.parse_args()
    
    key = sha256.hash(args.password.encode('utf-8'))
    if args.encrypt:
        if args.infile:
        ...
        if args.use_ecb:
            ...
            init_vector = int(args.initialisation_vector, 16)
            ciphertext = encrypt_cbc(plaintext, key, init_vector)
        
        if args.hex:
            ciphertext = hex(int.from_bytes(ciphertext, 'big'))
        
        if args.outfile:    
            with open(args.outfile, 'w' if args.hex else 'wb') as f:
                f.write(ciphertext)
        else:
            print(ciphertext)
    else:
        if args.infile:
            if args.hex:
                with open(args.infile, 'r') as f:
                    ciphertext = i_to_b(int(f.read(), 16))
            else:
                with open(args.infile, 'rb') as f:
                    ciphertext = f.read()
        else:
            if args.hex:
                ciphertext = i_to_b(int(input(), 16))
            else:
                raise("Cannot read raw bytes from stdin.")
        
        if args.use_ecb:
            plaintext = decrypt_ecb(ciphertext, key)
        else:
            init_vector = int(args.initialisation_vector, 16)
            plaintext = decrypt_cbc(ciphertext, key, init_vector)
